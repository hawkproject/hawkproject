...
namespace Hawk
{
	...
	{
	public:
		HawkApp_TickTask(const XID& sXid = XID(0,0)) : m_iLock(0), m_sXid(sXid)
		{			
		};

		HawkApp_TickTask(const XIDVector& vXid) : m_iLock(0)
		{
			for (Size_t i=0;i<vXid.size();i++)
			{
				m_vXid.push_back(vXid[i]);
	...
	public:
		virtual Bool  Lock()
		{
			if (HawkAtomic::CompareExchange<UInt32>(&m_iLock, 1, 0) == 0)
				return true;
		...
		virtual Bool  Unlock()
		{
			if (HawkAtomic::CompareExchange<UInt32>(&m_iLock, 0, 1) == 1)
				return true;
	...
	class HawkApp_ExceptionTask : public HawkTask
	{
	public:
		HawkApp_ExceptionTask(const XID& sXid = XID(0,0), Int32 iCode = 0) : m_iCode(iCode), m_sXid(sXid)
		{			
		};

		HawkApp_ExceptionTask(const XIDVector& vXid, Int32 iCode = 0) : m_iCode(iCode)
		{
			for (Size_t i=0;i<vXid.size();i++)
			{
				m_vXid.push_back(vXid[i]);
	...
	public:
		...
		virtual PVoid  Run(void* pArgs = 0)
		{
			if (m_vXid.size())
			{
				for (Size_t i=0;i<m_vXid.size();i++)
				{
					g_HawkApp->DispatchException(m_vXid[i], m_iCode);
				}
			}
			else
			{
				g_HawkApp->DispatchException(m_sXid, m_iCode);
			}
			return 0;
		}
	...
	Bool HawkApp::PostException(const XIDVector& vXID, Int32 iCode)
	{
		Int32 iCount = GetThreadNum();
		HawkAssert(iCount > 0);
		map<Int32, XIDVector> mThreadXid;
		...
		for (;it!=mThreadXid.end();it++)
		{							
			HawkApp_ExceptionTask* pTask = new HawkApp_ExceptionTask(it->second, iCode);
			HawkScope::ObjPtr scope(pTask);				
			PostAppTask(pTask, it->first);
		}
	...
	Bool HawkApp::DispatchException(const XID& sXid, Int32 iCode)
	{
		if(sXid.IsValid())
		{
			SafeObj obj(sXid);
			if (obj.IsObjValid())
				return obj->OnException(iCode);		
		}
		return false;
	}
}
