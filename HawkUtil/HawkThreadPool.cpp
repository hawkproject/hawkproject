...
namespace Hawk
{
	...
	Bool HawkThreadPool::IsWaitBreak() const
	{
		return m_bWaitBreak;
	}
	...
	{
		if (!m_bRunning || m_bWaitBreak || !pTask || !m_iThreadNum)
			return false;
	...
	void HawkThreadPool::Close(Bool bWaitBreak)
	{
		if (m_bRunning && !m_bWaitBreak)
		{
			m_bWaitBreak = bWaitBreak;
			if (!bWaitBreak)
				m_bRunning = false;

			for (Size_t i=0;i<m_vThread.size();i++)
			{
				m_vThread[i]->Close(bWaitBreak);
			}
			m_bRunning   = false;
			m_bWaitBreak = false;
